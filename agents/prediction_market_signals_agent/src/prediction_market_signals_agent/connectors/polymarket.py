...
from prediction_market_research_agent.clients.adapter import polymarket_yes_token_id
...
class PolymarketConnector(PredictionMarketConnector):
    ...
    async def search_markets(self, query: str, *, limit: int = 50) -> list[NormalizedMarket]:
        markets = await self._research.list_polymarket_markets(limit=limit)
        q = query.lower()
        results = []
        for m in markets:
    ...
    async def get_market(self, market_id: str) -> NormalizedMarket:
        m = await self._research.get_polymarket_market(market_id)
    ...
    async def get_orderbook(self, market_id: str) -> NormalizedOrderBook | None:
        async with httpx.AsyncClient(timeout=self._settings.http_timeout_seconds) as client:
            ...
            if not token_id:
                return None
            resp = await client.get(f"{self._clob}/book", params={"token_id": token_id})
